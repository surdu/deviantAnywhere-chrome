pref("deviantAnywhere.lastMessages", "");

pref("deviantAnywhere.autoupdate", true);
pref("deviantAnywhere.checktime", 300);
pref("deviantAnywhere.openMsgOnNew", false);
pref("deviantAnywhere.openNotification", true);
pref("deviantAnywhere.playsound", true);
pref("deviantAnywhere.sound", "chrome://devany/content/sounds/splat.wav");
pref("deviantAnywhere.customLook", true);
pref("deviantAnywhere.bkgcolor", "#728776");
pref("deviantAnywhere.textcolor", "#FFFFFF");
pref("deviantAnywhere.showIcon", true);

pref("deviantAnywhere.followNotices", true);
pref("deviantAnywhere.followActivity", true);
pref("deviantAnywhere.followReplies", true);
pref("deviantAnywhere.followNotes", true);
pref("deviantAnywhere.followJournals", true);
pref("deviantAnywhere.followComments", true);
pref("deviantAnywhere.followDeviations", true);
pref("deviantAnywhere.followPolls", true);
pref("deviantAnywhere.followContest", true);
pref("deviantAnywhere.followNews", true);
pref("deviantAnywhere.followAdmin", true);
pref("deviantAnywhere.followCritiques", true);
pref("deviantAnywhere.followCorrespondence", true);

